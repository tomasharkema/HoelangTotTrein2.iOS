...
private func FareTimeView(fareTime: FareTime, font: Font = Font.body) -> Text {
  let formatter = DateFormatter()
  formatter.dateFormat = "HH:mm"
...
func IfLet<ValueType, TrueContent: View, FalseContent: View>(_ value: ValueType?, _ yay: (ValueType) -> TrueContent, _ nay: () -> FalseContent) -> some View {
  Group {
    if value != nil {
      yay(value!)
    } else {
