...
struct TickerView: UIViewRepresentable {
  ...
  func makeUIView(context: UIViewRepresentableContext<TickerView>) -> UITickerView {
  ...
  func updateUIView(_ uiView: UITickerView, context: UIViewRepresentableContext<TickerView>) {
    uiView.date = date
    uiView.fontSize = fontSize
  }

  static func dismantleUIView(_ uiView: UITickerView, coordinator: ()) {
    uiView.stop()
  }
}
...
class UITickerView: UILabel {
  ...
  @objc func step() {
    let offset = self.date.timeIntervalSinceNow
    let text: String
    if offset > 0 {
      text = self.formatter.string(from: self.date.timeIntervalSinceNow) ?? "0"
    } else {
      text = "0"
    }
    self.text = text
