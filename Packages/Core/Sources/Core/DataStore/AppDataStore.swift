...
public class AppDataStore: DataStore {
  ...
  public init(useInMemoryStore: Bool = false, defaultKeepDepartedAdvice: Bool) {
    ...
    #if os(watchOS)
      let bundleIdentifier = "io.harkema.CoreWatch"
    #else
      let bundleIdentifier = "io.harkema.Core"
    #endif

    guard let url = Bundle.module.url(forResource: "HoelangTotTrein2", withExtension: "momd"),
  ...
  private func fetchMostUsedDict() -> Promise<[(UicCode, Int)], Error> {
    ...
    persistentContainer.performBackgroundTask { context in
      ...
      guard let entity = NSEntityDescription.entity(forEntityName: "History", in: context),
        let stationCode = entity.attributesByName[stationCodeKeyPath]
      else {
...
extension History {
  var historyType: HistoryType! {
    get {
      HistoryType(rawValue: type!.intValue)
    }
    set {
