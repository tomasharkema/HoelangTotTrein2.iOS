...
public struct MultipleAdvices {
  ...
  public var trips: Advices {
    Array(responses.map(\.trips).joined())
  }
}
...
public class TravelService: NSObject {
  ...
  public var adviceStations: Variable<AdviceStations> { adviceStationsSource.variable }
  ...
  public var error: Channel<Error> { errorSource.channel }
...
//      }
  ...
  public func tick(userInteraction: Bool) -> Promise<Void, Error> {
    fetchCurrentAdvices(shouldEmitLoading: userInteraction)
      .finallyResult {
  ...
  public func fetchStations() -> Promise<Stations, Error> {
    apiService.stations(cancellationToken: nil)
      .mapError { $0 as Error }
...
//    }
  ...
  public func find(stationNameContains: String) -> Promise<[Station], Error> {
    dataStore.find(stationNameContains: stationNameContains)
  }

  public func find(stationCode: StationCode) -> Promise<Station, Error> {
    dataStore.find(stationCode: stationCode)
  }
}
